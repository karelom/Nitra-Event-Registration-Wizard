...
| 2026-06-19 | ~0.2h   | Mobile-responsive layout (375px minimum): responsive padding (`px-4 sm:px-8 md:px-[120px]`) across all step containers and footer; stacked ticket cards and form field rows on mobile (`flex-col sm:flex-row`); single-column session grid on mobile (`grid-cols-1 sm:grid-cols-2`); Step3 sidebar stacks below content on mobile (`flex-col md:flex-row`, `w-full md:w-[380px]`); reduced SuccessState padding on mobile; stepper header shows only active step tab on mobile via CSS (`:not(.q-stepper__tab--active)` hidden) |
