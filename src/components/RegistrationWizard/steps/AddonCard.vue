...
const isSoldOut = computed(() =>
  props.addon.category === 'workshop' &&
  props.addon.capacity !== undefined &&
  props.addon.registered !== undefined &&
  props.addon.registered >= props.addon.capacity
)
...
const isMerch = computed(() => props.addon.category === 'merchandise')
...
const cardClass = computed(() => {
  const radius = isMerch.value ? 'rounded-lg' : 'rounded-[6px]'
  const borderWidth = isMerch.value ? 'border' : 'border-2'

  if (isDisabled.value) {
    return `${radius} ${borderWidth} bg-surface-l2 border-neutral-muted cursor-not-allowed`
  }
  if (props.selected) {
    return `${radius} ${borderWidth} bg-brand-subtle-rest border-brand-emphasis${isMerch.value ? '' : ' cursor-pointer'}`
  }
  return `${radius} ${borderWidth} bg-surface-l0 border-neutral-muted${isMerch.value ? '' : ' cursor-pointer hover:border-brand-muted'}`
})

function handleCardClick() {
  if (isDisabled.value || isMerch.value) return
  emit('toggle')
}
  ...
  >
    ...
    <div class="flex items-start justify-between gap-4">
      ...
      <p
        ...
        :class="addon.category === 'workshop' ? 'text-brand' : 'text-neutral'"
      >
        ${{ addon.price }}
      </p>
    </div>
    ...
    <!-- Workshop: time range + availability status -->
    <template v-if="addon.category === 'workshop' && addon.date && addon.endDate">
      <p
      ...
      <p class="text-[11px] leading-[14px] m-0 font-semibold">
    ...
    <!-- Merchandise: controls always visible -->
    <template v-if="isMerch">
